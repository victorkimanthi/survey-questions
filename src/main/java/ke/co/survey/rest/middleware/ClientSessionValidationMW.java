...
public class ClientSessionValidationMW extends ExchangeMiddleware {
    private final ExchangeHandler nextHandler;

    public ClientSessionValidationMW(ExchangeHandler nextHandler) {
...
/*
    ...
    public static TransactionWrapper<AccessTokenValidity> validateToken(String organizationId, String accessToken, String clientIPAddress) {
        ...
        try {
            ...
            long delay = getDelay(tokenDetails.getValue("time_unit"),Long.parseLong(tokenDetails.getValue("time_to_live").toString()));
    ...
    public static TransactionWrapper<FlexicoreHashMap> fetchTokenDetails(String organizationId, String accessToken) {
        try {
            QueryBuilder queryBuilder = new QueryBuilder()
                    ...
                    .from()
                    .joinPhrase("clients.access_tokens at LEFT JOIN clients.user_accounts ua on ua.user_id = at.user_id")
            ...
            FlexicoreHashMap userAccountDetails;
            {
                QueryBuilder queryBuilder = new QueryBuilder()
                        ...
                        .selectColumn("""
                                ua.*""")
                        .from()
                        .joinPhrase("clients.user_accounts as ua LEFT JOIN  clients.access_tokens at on ua.user_id = at.user_id")
                        .where("at.access_token = :access_token");
            ...
            if (userAccountDetails != null && !userAccountDetails.isEmpty()) {
                QueryBuilder queryBuilder = new QueryBuilder()
                        ...
                        .selectColumn("""
                                ap.*""")
                        .from()
                        .joinPhrase( "clients.user_accounts ua\n" +
                                "         LEFT JOIN clients.authentication_profiles ap on ua.authentication_profile_code = ap.profile_code")
    ...
    public static TransactionWrapper<FlexicoreHashMap> revokeToken(String organizationId, String accessToken) {
        ...
        TransactionWrapper<FlexicoreArrayList> transactionWrapper = Repository.delete(organizationId, "clients.access_tokens",
