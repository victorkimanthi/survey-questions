...
public class SessionValidationMW extends ExchangeMiddleware {
    private final ExchangeHandler nextHandler;

    public SessionValidationMW(ExchangeHandler nextHandler) {
...
/*
    ...
    public static TransactionWrapper<FlexicoreHashMap> fetchTokenDetails(String organizationId, String accessToken) {
        try {
            QueryBuilder queryBuilder = new QueryBuilder()
                    ...
                    .from()
                    .joinPhrase("master.access_tokens at LEFT JOIN master.user_accounts ua on ua.user_id = at.user_id")
            ...
            FlexicoreHashMap userAccountDetails;
            {
                QueryBuilder queryBuilder = new QueryBuilder()
                        ...
                        .selectColumn("""
                                ua.*""")
                        .from()
                        .joinPhrase("master.user_accounts as ua LEFT JOIN  master.access_tokens at on ua.user_id = at.user_id")
                        .where("at.access_token = :access_token");
            ...
            if (userAccountDetails != null && !userAccountDetails.isEmpty()) {
                QueryBuilder queryBuilder = new QueryBuilder()
                        ...
                        .selectColumn("""
                                ap.*""")
                        .from()
                        .joinPhrase( "master.user_accounts ua\n" +
                                "         LEFT JOIN master.authentication_profiles ap on ua.authentication_profile_code = ap.profile_code")
    ...
    public static TransactionWrapper<FlexicoreHashMap> revokeToken(String organizationId, String accessToken) {
        ...
        TransactionWrapper<FlexicoreArrayList> transactionWrapper = Repository.delete(organizationId, "master.access_tokens",
